...
async (req, res) => 
{
    try 
    {
        // 'populate' can join fields from other collections
...
async (req, res) =>
{
    ...
    try 
    {
        ...
        if (profile != null)
        {
            // 'unshift' adds new element at beginning of array
        ...
        {
            res.status(500).send("Server Error: Profile for this user doesn't exist");
        }
...
async (req, res) =>
{
    try 
    {
        // need to handle cases for null 'profile' and 'removeIndex'
        const profile = await Profile.findOne({user: req.user.id});
